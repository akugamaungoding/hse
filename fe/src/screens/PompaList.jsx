...
import { Search, QrCode, CheckCircle2, AlertTriangle, Droplets } from "lucide-react";
...
export function PompaList() {
  const { pompas } = useApp();
  const [query, setQuery] = useState("");
  const [filterStatus, setFilterStatus] = useState("Semua");

  const filtered = pompas.filter((p) => {
    const matchQuery =
  ...
  return (
    <PageLayout title="Pompa Hydrant">
      <div className="p-4 flex flex-col gap-4 pb-20 relative">
        {/* PIC Banner */}
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider">
              PIC Pompa Hydrant (Semua Pompa)
            </span>
            <p className="text-xs font-bold text-gray-800">
              Tim Utilitas & Unit K3 (Bpk. Hendra & Tim Damkar)
            </p>
          </div>
          <span className="text-[10px] bg-blue-600 text-white font-bold px-2.5 py-1 rounded-full">
            Cakup All Pumps
          </span>
        </div>
        ...
        <p className="text-xs text-gray-400">
          Menampilkan {filtered.length} dari {pompas.length} pompa
        </p>

        <div className="flex flex-col gap-3">
          {filtered.length === 0 ? (
            <p className="text-center text-sm text-gray-400 py-8">Tidak ada pompa yang cocok</p>
          ) : (
            filtered.map((pom) => (
              <Link
                key={pom.id}
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                    {statusIcon(pom.status)}
                    <div>
                      <h3 className="font-['Poppins',sans-serif] font-bold text-[#0140c7]">
                        {pom.id}
                      </h3>
                      <p className="text-xs text-gray-500">{pom.lokasi}</p>
                    </div>
                  </div>
                  <span
                ...
                <div className="flex justify-between items-end border-t border-gray-50 pt-2">
                  <div className="flex items-center gap-1.5 min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-600 truncate">{pom.detail}</p>
                  </div>
                  <p className="text-[10px] text-gray-400 shrink-0 ml-2">
                    Terakhir: {pom.lastInspeksi}
                  </p>
                </div>
              </Link>
            ))
